        ...
        {(data ?? []).map((n) => (
          <li key={n.id} className={`flex items-center justify-between rounded-xl border border-border/60 p-4 ${n.read_at ? "bg-card/30 opacity-60" : "bg-card/60"}`}>
            <div>
              ...
              <div className="mt-1 text-xs text-muted-foreground">{new Date(n.created_at).toLocaleString(locale)}</div>
            </div>
            <div className="flex items-center gap-2">
              {n.chapters?.chapter_url && (
                <a href={n.chapters.chapter_url} target="_blank" rel="noopener noreferrer"
                  className="rounded-md bg-secondary/60 px-3 py-1.5 text-xs hover:bg-secondary">{t("common.open")}</a>
              )}
              {!n.read_at && (
                <button onClick={() => markRead.mutate(n.id)} className="rounded-md p-1.5 hover:bg-accent/10" title={t("notif.markRead")}>
